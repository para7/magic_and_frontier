...
execute unless score @s mafMP matches 100.. run kill @e[type=arrow,distance=..2,tag=maf_bow_arrow_new]
execute unless score @s mafMP matches 100.. run tellraw @s [{"text":"MPが足りません！ 消費MP: 100"}]
execute unless score @s mafMP matches 100.. run playsound minecraft:block.dispenser.fail master @s ~ ~ ~ 1.0 1.1
execute unless score @s mafMP matches 100.. run return 0
scoreboard players remove @s mafMP 100
