...
func (a App) enemySkillsPage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		a.renderEnemySkills(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Notice: errorNotice(err.Error())})
		return
	}
	a.renderEnemySkills(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Entries: state.Entries, Notice: notice, Form: defaultEnemySkillForm()})
}
...
func (a App) enemySkillsNewPage(w http.ResponseWriter, r *http.Request) {
	...
	a.renderEnemySkillForm(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Form: form})
}
...
func (a App) enemySkillsEditPage(w http.ResponseWriter, r *http.Request) {
	...
	a.renderEnemySkills(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Entries: state.Entries, Notice: errorNotice("Enemy skill not found.")})
}
...
func (a App) enemySkillsSave(w http.ResponseWriter, r *http.Request, editing bool) {
	...
	if err != nil {
		...
		form.ReturnTo = returnTo
		a.renderEnemySkillForm(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Notice: errorNotice(err.Error()), Form: form})
		return
	}
	state, err := a.loadEnemySkillStateFromMaster()
	if err != nil {
	...
	a.renderEnemySkills(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Entries: nextState.Entries, Notice: notice})
}
...
func (a App) enemySkillsDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		a.renderEnemySkills(w, r, ui.EnemySkillsPageData{Meta: enemySkillsMeta(), Entries: state.Entries, Notice: errorNotice(err.Error())})
		return
	}
...
func (a App) renderEnemySkills(w http.ResponseWriter, r *http.Request, data ui.EnemySkillsPageData) {
...
func (a App) renderEnemySkillForm(w http.ResponseWriter, r *http.Request, data ui.EnemySkillsPageData) {
	data.Meta = applyPageMeta(r, data.Meta)
	if isHX(r) {
...
func enemySkillsMeta() ui.PageMeta {
	return ui.PageMeta{Title: "Enemy Skills", CurrentPath: "/enemy-skills"}
}

func defaultEnemySkillForm() ui.EnemySkillFormData {
	return ui.EnemySkillFormData{
		FieldErrors: map[string]string{},
	}
}

func enemySkillEntryToForm(entry enemyskills.EnemySkillEntry) ui.EnemySkillFormData {
	return ui.EnemySkillFormData{
		ID:          entry.ID,
		Name:        entry.Name,
...
func parseEnemySkillForm(r *http.Request) (ui.EnemySkillFormData, enemyskills.SaveInput, map[string]string) {
	form := defaultEnemySkillForm()
...
func enemySkillOptions(entries []enemyskills.EnemySkillEntry) []ui.ReferenceOption {
	options := make([]ui.ReferenceOption, 0, len(entries))
	for _, entry := range entries {
		options = append(options, ui.ReferenceOption{ID: entry.ID, Label: entry.Name})
	}
	return options
}
