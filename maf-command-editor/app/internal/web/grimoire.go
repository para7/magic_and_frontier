...
func (a App) grimoirePage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		a.renderGrimoire(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Notice: errorNotice(err.Error())})
		return
	}
	a.renderGrimoire(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Entries: state.Entries, Notice: notice, Form: defaultGrimoireForm(nil)})
}
...
func (a App) grimoireNewPage(w http.ResponseWriter, r *http.Request) {
	...
	a.renderGrimoireForm(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Form: form})
}
...
func (a App) grimoireEditPage(w http.ResponseWriter, r *http.Request) {
	...
	a.renderGrimoire(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Entries: state.Entries, Notice: errorNotice("Grimoire entry not found.")})
}
...
func (a App) grimoireSave(w http.ResponseWriter, r *http.Request, editing bool) {
	...
	if err != nil {
		...
		form.ReturnTo = returnTo
		a.renderGrimoireForm(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Notice: errorNotice(err.Error()), Form: form})
		return
	}
	state, err := a.loadGrimoireStateFromMaster()
	if err != nil {
	...
	a.renderGrimoire(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Entries: nextState.Entries, Notice: notice})
}
...
func (a App) grimoireDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := master.Grimoires().Save(); err != nil {
		a.renderGrimoire(w, r, views.GrimoirePageData{Meta: grimoireMeta(), Entries: state.Entries, Notice: errorNotice(err.Error())})
		return
	}
	nextState, err := a.loadGrimoireStateFromMaster()
	if err != nil {
...
func (a App) renderGrimoire(w http.ResponseWriter, r *http.Request, data views.GrimoirePageData) {
...
func (a App) renderGrimoireForm(w http.ResponseWriter, r *http.Request, data views.GrimoirePageData) {
	data.Meta = applyPageMeta(r, data.Meta)
	if isHX(r) {
...
func grimoireMeta() views.PageMeta {
	return views.PageMeta{Title: "Grimoire", CurrentPath: "/grimoire"}
}

func defaultGrimoireForm(_ []grimoire.GrimoireEntry) views.GrimoireFormData {
	return views.GrimoireFormData{
		ID:          "",
		CastTime:    "0",
		MPCost:      "0",
...
func grimoireEntryToForm(entry grimoire.GrimoireEntry) views.GrimoireFormData {
	return views.GrimoireFormData{
		ID:          entry.ID,
...
func parseGrimoireForm(r *http.Request) (views.GrimoireFormData, grimoire.SaveInput, map[string]string) {
	form := defaultGrimoireForm(nil)
...
func grimoireOptions(entries []grimoire.GrimoireEntry) []views.ReferenceOption {
	options := make([]views.ReferenceOption, 0, len(entries))
	for _, entry := range entries {
		options = append(options, views.ReferenceOption{ID: entry.ID, Label: entry.Title})
	}
	return options
}
