...
この文書は、現状の `tools2` を「JSON をテーブル化して見た場合」の疑似 SQL で整理したものです。
実 DB は存在せず、実際の保存先は `savedata/*.json` ですが、画面/API/検証/export のつながりを追いやすくするために SQL 風に表現します。
...
CREATE TABLE items (
  id UUID PRIMARY KEY,
  item_id TEXT,           -- Minecraft item id
  updated_at TEXT
);
```
