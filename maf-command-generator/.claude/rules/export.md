...
| `convert/` | 純粋変換関数群（`export_convert` パッケージ。grimoire・item・loot table・enemy 変換） |
...
| `grimoire.go` | `GrimoireToBook` | グリモア → 本 NBT 変換 |
| `item.go` | なし | item カスタムデータ・コンポーネント・エンチャント（内部） |
...
- 変換専用の純粋関数は `convert/` サブパッケージ（`export_convert`）に置く（例: `GrimoireToBook`, `BuildDropLootPool`）
- `export` パッケージからは `exportconvert "maf_command_editor/app/domain/export/convert"` でインポートする
